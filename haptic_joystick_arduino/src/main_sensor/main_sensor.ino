...
// LOAD SENSOR calibration and force computation
float mean_offset[3] = {880000,-1100000,700000};
float factor_calibration[] = {20000, 20000, 20000} ;
...
// Declare class to connect to Qwicmux and load cell NAU7802
QWIICMUX myMux;
NAU7802 load_sensor;
...
void setup() {
  ...
  // Check that the 3 load cell are correctly detected on  the QWIICMUX
  Serial.begin(115200);
...
void loop() {
  ...
  if (dt_iter >= 1000) {
    ...
    int32_t currentReading_1 = load_sensor.getReading();
    force_1 = (currentReading_1 - mean_offset[0])/factor_calibration[0]*g;
    myMux.setPort(2); // Connect master to port labeled '2' on the mux for the Qwiic Scale
    int32_t currentReading_2 = load_sensor.getReading();
    force_2 = (currentReading_2 - mean_offset[1])/factor_calibration[1]*g;
    myMux.setPort(3); // Connect master to port labeled '2' on the mux for the Qwiic Scale
    int32_t currentReading_3 = load_sensor.getReading();
    force_3 = (currentReading_3 - mean_offset[2])/factor_calibration[2]*g;
    ...
    if (Serial.available() > 0) {
      ...
      if (Serial.find("<") == true) {
        ...
        // Depending on the other module, if the message [-1,-1;-1] it will consider no change on load cell so no changement of positions required
        if (data_teensy == "-1;-1;-1"){
          desired_pos[0] = encoder_count_A;
          desired_pos[1] = encoder_count_B;
          desired_pos[2] = encoder_count_C;
        }
        else {
        // If there is something from load cells change the desired_position to the position send by ROS
    ...
    String output_data = "<" + String(encoder_count_A) + ";" + String(encoder_count_B) + ";" + String(encoder_count_C) + ";" + String(refA) + ";" + String(refB) + ";" + String(refC) +";" + String(force_1) + ";" + String(force_2) + ";" + String(force_3) +">";
...
///////////////////    //  Serial.print(pwmA);Serial.print("\t");
    //  Serial.print(encoder_count_B);Serial.print("\t");/////////Functions////////////////////////////
/////////////////////////////////////////////////////////////////
