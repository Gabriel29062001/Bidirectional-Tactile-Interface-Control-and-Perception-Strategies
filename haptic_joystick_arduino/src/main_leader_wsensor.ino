...
void setup() {
  ...
  for (int i = 0; i < 3; i++ ){
    ...
    for (int j = 0; j < iterations; j++) {
      ...
      int32_t currentReading = load_sensor.getReading();
      offset[i] = offset[i] + currentReading;
      
    }
    mean_offset[i] = offset[i]/iterations;
  }    
  while (Serial.available()) Serial.read(); //Clear anything in RX buffer
  while (Serial.available() == 0){
    Serial.println("Press enter to start the calibration");
    delay(10); 
  }
  for (int i = 0; i < 3; i++ ){
    myMux.setPort(i+1); 
    while (Serial.available()) Serial.read(); //Clear anything in RX buffer
    while (Serial.available() == 0){
      Serial.print("Put a weight on sensor ");
      Serial.println(i+1);
      delay(10); 
    }
    for (int j = 0; j < iterations; j++) {
      ...
      int32_t currentReading = load_sensor.getReading();
      calibration[i] = calibration[i] + currentReading;
      
    }
    factor_calibration[i] = (calibration[i]/iterations-mean_offset[i])/weight;
  }   
