...
float vicon_motor_1[] = {40, 19.68, 40, 0, 0, 0 };
float vicon_motor_2[] = {40, 19.68, 0, 40, 0, 0 };
float vicon_motor_3[] = {40, 19.68, 0, 0, 40, 0 };
int i = 0;
...
void loop() {
  ...
  if (dt_iter >= 1000) {
    ...
    refA = vicon_motor_1[i];
    refB = vicon_motor_2[i];
    refC = vicon_motor_3[i];
    ...
    if (abs(errA) < 10 && abs(errB) < 10 && abs(errC) < 10){
      i = i+1;
      delay(5000);
    }
